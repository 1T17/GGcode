...
ASTResult parse_source(const char *source)
{
    ...
    strncpy(RUNTIME_FILENAME, "<test_input>", sizeof(RUNTIME_FILENAME) - 1);
    ASTResult result = {0}; // Ensure initialized

    if (setjmp(fatal_error_jump_buffer)) {
        // If a fatal error occurred, stop and return NULL root
        result.root = NULL;
        return result;
    }
...
{
    const char *source = "function foo(a, b) { return a + b }";
    ASTResult result = parse_source(source);
    ...
    TEST_ASSERT_EQUAL_STRING("foo", func->function_stmt.name);
    TEST_ASSERT_EQUAL(2, func->function_stmt.param_count);
    TEST_ASSERT_EQUAL_STRING("a", func->function_stmt.params[0]);
    TEST_ASSERT_EQUAL_STRING("b", func->function_stmt.params[1]);
    ...
    ASTNode *ret = body->block.statements[0];
    TEST_ASSERT_EQUAL(AST_RETURN, ret->type);
    TEST_ASSERT_EQUAL(AST_BINARY, ret->return_stmt.expr->type);
...
{
    const char *source = "let x = foo(1, 2)";
    ASTResult result = parse_source(source);
    ...
    ASTNode *stmt = result.root->block.statements[0];
    TEST_ASSERT_EQUAL(AST_LET, stmt->type);
    TEST_ASSERT_EQUAL_STRING("x", stmt->let_stmt.name);

    ASTNode *rhs = stmt->let_stmt.expr;
    TEST_ASSERT_NOT_NULL(rhs);
    TEST_ASSERT_EQUAL(AST_CALL, rhs->type);
    TEST_ASSERT_EQUAL_STRING("foo", rhs->call_expr.name);
    TEST_ASSERT_EQUAL(2, rhs->call_expr.arg_count);

    ASTNode *arg1 = rhs->call_expr.args[0];
    ASTNode *arg2 = rhs->call_expr.args[1];

    TEST_ASSERT_EQUAL(AST_NUMBER, arg1->type);
    TEST_ASSERT_EQUAL_FLOAT(1.0, arg1->number.value);
    TEST_ASSERT_EQUAL(AST_NUMBER, arg2->type);
    TEST_ASSERT_EQUAL_FLOAT(2.0, arg2->number.value);
...
{
    const char *source = "let result = foo(1, 2, 3)";
    ASTResult result = parse_source(source);

    ASTNode *let_stmt = result.root->block.statements[0];
    TEST_ASSERT_EQUAL(AST_LET, let_stmt->type);
    TEST_ASSERT_EQUAL_STRING("result", let_stmt->let_stmt.name);

    ASTNode *call = let_stmt->let_stmt.expr;
    TEST_ASSERT_EQUAL(AST_CALL, call->type);
    TEST_ASSERT_EQUAL_STRING("foo", call->call_expr.name);
    TEST_ASSERT_EQUAL_INT(3, call->call_expr.arg_count);

    // Check each argument
    TEST_ASSERT_EQUAL(AST_NUMBER, call->call_expr.args[0]->type);
    TEST_ASSERT_EQUAL_DOUBLE(1.0, call->call_expr.args[0]->number.value);

    TEST_ASSERT_EQUAL(AST_NUMBER, call->call_expr.args[1]->type);
    TEST_ASSERT_EQUAL_DOUBLE(2.0, call->call_expr.args[1]->number.value);

    TEST_ASSERT_EQUAL(AST_NUMBER, call->call_expr.args[2]->type);
    TEST_ASSERT_EQUAL_DOUBLE(3.0, call->call_expr.args[2]->number.value);
...
void test_standalone_function_call(void) {
    const char *source = "foo(1, 2)";
    ASTResult result = parse_source(source);

    TEST_ASSERT_NOT_NULL(result.root);
    TEST_ASSERT_EQUAL(AST_BLOCK, result.root->type);
    TEST_ASSERT_TRUE(result.root->block.count > 0);
    TEST_ASSERT_NOT_NULL(result.root->block.statements);

    ASTNode *stmt = result.root->block.statements[0];
    TEST_ASSERT_NOT_NULL(stmt);
    TEST_ASSERT_EQUAL(AST_EXPR_STMT, stmt->type);

    ASTNode *call = stmt->expr_stmt.expr;
    TEST_ASSERT_NOT_NULL(call);
    TEST_ASSERT_EQUAL(AST_CALL, call->type);
    TEST_ASSERT_EQUAL_STRING("foo", call->call_expr.name);
    TEST_ASSERT_EQUAL_INT(2, call->call_expr.arg_count);

    TEST_ASSERT_NOT_NULL(call->call_expr.args[0]);
    TEST_ASSERT_NOT_NULL(call->call_expr.args[1]);
    TEST_ASSERT_EQUAL(AST_NUMBER, call->call_expr.args[0]->type);
    TEST_ASSERT_EQUAL(AST_NUMBER, call->call_expr.args[1]->type);
    TEST_ASSERT_EQUAL_DOUBLE(1.0, call->call_expr.args[0]->number.value);
    TEST_ASSERT_EQUAL_DOUBLE(2.0, call->call_expr.args[1]->number.value);
...
RUN_TEST(test_standalone_function_call);



   RUN_TEST(test_function_definition);
